...
- (void) getAllContactsCount:(RCTResponseSenderBlock) callback
{
    ...
    NSNumber *count = [NSNumber numberWithInteger:contactsCount];
    ...
    for (CNLabeledValue<CNInstantMessageAddress*>* labeledValue in person.instantMessageAddresses) {
        ...
        if (instantMessageAddress) {
            ...
            if (instantMessageAddress.service) {
                [dict setObject: instantMessageAddress.service forKey:@"service"];
            }
            if (instantMessageAddress.username) {
                [dict setObject:instantMessageAddress.username forKey:@"username"];
            }
            [instantMessageAddresses addObject:dict];
        }
    }
    [output setObject:instantMessageAddresses forKey:@"instantMessageAddresses"];
...
- (void) updateRecord:(CNMutableContact *)contact withData:(NSDictionary *)contactData
{
    ...
    NSMutableArray *instantMessageAddresses = [[NSMutableArray alloc] init];
    for (id instantMessageAddressData in [contactData valueForKey:@"instantMessageAddresses"]) {
        NSString *label = [instantMessageAddressData valueForKey:@"label"];
        NSString *service = [instantMessageAddressData valueForKey:@"service"];
        NSString *username = [instantMessageAddressData valueForKey:@"username"];
        CNInstantMessageAddress *instantMessageAddress = [[CNInstantMessageAddress alloc] initWithUsername:username service:service];
